-- Create stored functions for feedback generation
-- These functions are required for generating LLM-based competency feedback

-- Function 1: get_competency_results (for known_roles and all_roles survey types)
-- This overload accepts a username instead of user_id
...
-- Function 2: get_unknown_role_competency_results (for unknown_roles/task-based survey type)
...
BEGIN
    ...
    competency_indicators_with_score AS (
        SELECT competency_id, "level",
               CASE
                   WHEN "level" = 'kennen' THEN 1  -- FIXED: Was p_organization_id, should be 1
                   WHEN "level" = 'verstehen' THEN 2
                   WHEN "level" = 'anwenden' THEN 4
                   WHEN "level" = 'beherrschen' THEN 6
               END AS level_assigned_score,
